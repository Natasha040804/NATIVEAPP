...
const api = {
  async request(endpoint, options = {}) {
    ...
    try {
      ...
      if (ms > 1500) console.log(`⏱️ Slow API (${ms}ms) ${endpoint}`);
      return data;
    } catch (error) {
      const ms = Date.now() - startedAt;
      ...
      if (/ENOTFOUND|DNS|resolve/i.test(error.message)) {
        throw new Error(`DNS/Host resolution failed for ${API_BASE_URL} after ${ms}ms`);
      }
      if (/ECONNREFUSED|refused/i.test(error.message)) {
        throw new Error(`Connection refused at ${API_BASE_URL}. Is the port/proxy open? (${ms}ms)`);
      }
      if (/SSL|certificate|handshake/i.test(error.message)) {
        throw new Error(`TLS/SSL error contacting ${API_BASE_URL}: ${error.message}`);
      }
      throw error;
    } finally {
      clearTimeout(timeoutId);
  ...
  async getConversationMessages(conversationKey) {
    return this.request(`/messages/conversations/${encodeURIComponent(conversationKey)}`);
  },
...
// Deep connectivity diagnostics: probes health endpoints and returns structured result
export const diagnoseConnectivity = async () => {
  ...
  for (const h of variants) {
    ...
    try {
      ...
      if (res.ok) {
        results.push({ url: h, ok: true, status: res.status, ms: elapsed });
      } else {
        results.push({ url: h, ok: false, status: res.status, ms: elapsed, error: `HTTP ${res.status}` });
      }
    } catch (e) {
      const elapsed = Date.now() - start;
      results.push({ url: h, ok: false, status: null, ms: elapsed, error: e?.message || String(e) });
    }
  }
  // Perform a lightweight external reachability check (captive portal style)
  let external = null;
  try {
    const extStart = Date.now();
    const extRes = await fetch('https://www.google.com/generate_204', { method: 'GET' });
    external = { ok: extRes.status === 204 || (extRes.status >= 200 && extRes.status < 300), status: extRes.status, ms: Date.now() - extStart };
  } catch (e) {
    external = { ok: false, status: null, error: e?.message || String(e) };
  }
  const classification = classifyConnectivity(results, external);
  return { base: API_BASE_URL, results, external, classification };
};

// Classify connectivity issues & produce remediation suggestions
export const classifyConnectivity = (results, external) => {
  const anyOk = results.some(r => r.ok);
  const allTimeout = results.every(r => /timed out|abort|network request failed/i.test(r.error || ''));
  const dns = results.find(r => /ENOTFOUND|DNS|resolve/i.test(r.error || ''));
  const refused = results.find(r => /ECONNREFUSED|refused/i.test(r.error || ''));
  const ssl = results.find(r => /SSL|certificate|handshake/i.test(r.error || ''));
  const externalFailed = external && !external.ok;
  let code = 'UNKNOWN';
  if (!anyOk && externalFailed) code = 'OFFLINE';
  else if (!anyOk && dns) code = 'DNS_FAILURE';
  else if (!anyOk && refused) code = 'CONNECTION_REFUSED';
  else if (!anyOk && ssl) code = 'SSL_ERROR';
  else if (!anyOk && allTimeout) code = 'TIMEOUT';
  else if (anyOk) code = 'PARTIAL_OK';
  const suggestions = {
    OFFLINE: [
      'Check device network/Wi‑Fi or mobile data',
      'Disable VPN/proxy that may block requests',
      'Try opening any website in browser to confirm internet'
    ],
    DNS_FAILURE: [
      'Verify API host/IP is correct',
      'Use raw IP instead of hostname to test',
      'Flush DNS cache / toggle airplane mode'
    ],
    CONNECTION_REFUSED: [
      'Confirm backend process is running (node server.js)',
      'Ensure firewall / security group opens port 80 or 5000',
      'Validate Nginx proxy forwards /api to backend'
    ],
    SSL_ERROR: [
      'Check certificate validity & expiration',
      'Use http:// for testing if SSL termination not ready',
      'Confirm device date/time is correct'
    ],
    TIMEOUT: [
      'Server may be overloaded or blocked',
      'Ping the server IP to measure latency',
      'Check reverse proxy upstream connectivity'
    ],
    PARTIAL_OK: [
      'Some endpoints healthy, others failing',
      'Inspect failing route logs for errors',
      'Check auth token validity if 401/403'
    ],
    UNKNOWN: [
      'Inspect device logs (LogBox / logcat)',
      'Re-run diagnostics after toggling network',
      'Verify latest app build includes updated API base URL'
    ]
  };
  return { code, suggestions: suggestions[code] };
};
